...
| Modo atual | **`SAAS_MODE=observe` em produção**. Isolamento de leitura + mutação preparados; **inertes** até `on`. |
| Fase em andamento | **Fase 2** — pronto para enforce gradual em `/api/restaurant-reservations`. `loadUserScope` traduz memberships → ids operacionais (place/bar). |
| Próximo passo | Validar logs `[tenant:observe]`; teste com usuário restrito + `SAAS_MODE=on` em janela controlada (ver `tenancy/README.md` runbook). |
| Pendências/decisões | JWT com tenant; RLS; Fase 4 (sidebar por módulo). `memberships` ainda vazio → enforce usa `user_establishment_permissions` (53 perms). |
...
- [~] **Fase 2** — Isolamento. Observe ativo; escopo em GET listagem/stats/:id/guest-list + **PUT/DELETE/link-event**; `loadUserScope` traduz ids canônicos→operacionais. Próximo: enforce gradual com usuário de teste (`SAAS_MODE=on`). Falta: JWT tenant; RLS; mais rotas.
