...
| Próximo passo | Aplicar migration 008 em staging; `SAAS_RLS_MODE=on` na API após validação; `NEXT_PUBLIC_SAAS_MODE=on` no front; expandir RLS a outras tabelas. |
| Pendências/decisões | Contract NOT NULL em `organization_id`; 8 promoters órfãos; mapping memberships→operacional antes de popular memberships. |
...
- [~] **Fase 2** — enforce app-layer + **JWT tenant** (`organization_id` no login) + **RLS piloto** migration 008 (`restaurant_reservations`, `SAAS_RLS_MODE`). Falta: expandir RLS; gaps stats.
...
- 2026-07-02 — **JWT + RLS piloto:** token com `organization_id`/`organization_ids`/`is_super_admin`; UEP resolve org via `establishments`; migration `008_rls_restaurant_reservations`; `SAAS_RLS_MODE` + pool wrap com AsyncLocalStorage. Ativar RLS só após migration 008 em staging.
