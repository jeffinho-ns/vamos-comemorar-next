...
export default function WeeklyCalendar({
  ...
  const getSeuJustinoAreaName = (
    ...
    if (
      areaName &&
      !areaName.toLowerCase().includes("área coberta") &&
      !areaName.toLowerCase().includes("área descoberta")
    ) {
      return areaName;
    }
    ...
    if (
      areaName &&
      (areaName.toLowerCase().includes("coberta") ||
        areaName.toLowerCase().includes("descoberta"))
    ) {
  ...
  const weekData = useMemo(() => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const dayReservations = reservations.filter((reservation) => {
        try {
          const normalizedReservationDate = normalizeDate(
            reservation.reservation_date,
          );
          return normalizedReservationDate === dayString;
        } catch (error) {
          console.error(
            "Erro ao processar data da reserva:",
            reservation.reservation_date,
            error,
          );
          return false;
      ...
      const timeSlots: TimeSlot[] = TIME_SLOTS.map((time) => {
        const slotReservations = dayReservations.filter((reservation) => {
          const normalizedReservationTime = normalizeTime(
  ...
  const weekStats = useMemo(() => {
    const totalReservations = weekData.reduce(
      (sum, day) => sum + day.totalReservations,
      0,
    );
    const totalPeople = weekData.reduce((sum, day) => sum + day.totalPeople, 0);
    // Calcular média dividindo pelo número de dias da semana (7)
    const averagePerDay =
      weekData.length > 0 ? totalReservations / weekData.length : 0;
      ...
      {process.env.NODE_ENV === "development" && (
        <div className="bg-blue-50 p-3 border-b border-blue-200">
          <div className="text-sm text-blue-700">
            <p>
              <strong>Total de reservas carregadas:</strong>{" "}
              {reservations.length}
            </p>
            <p>
              <strong>Reservas da semana atual:</strong>{" "}
              {weekStats.totalReservations}
            </p>
            <p>
              <strong>Semana exibida:</strong> {weekData[0]?.dayNumber}{" "}
              {weekData[0]?.monthName} - {weekData[6]?.dayNumber}{" "}
              {weekData[6]?.monthName} {weekData[6]?.date.getFullYear()}
            </p>
          </div>
        </div>
      )}
        ...
        <div className="flex items-center justify-between">
          ...
          {/* <div className="text-center">
            ...
            <div className="flex items-center gap-4 mt-2 text-sm text-gray-600">
              ...
              <div className="flex items-center gap-1">
                <MdBarChart size={16} />
                <span>Média: {weekStats.averagePerDay.toFixed(1)}/dia</span>
              </div>
            </div>
          </div> */}
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[1000px]">
            ...
            {TIME_SLOTS.map((time, timeIndex) => {
              const hasReservations = weekData.some(
                (day) => day.timeSlots[timeIndex]?.reservations.length > 0,
              );
                  ...
                  {weekData.map((day, dayIndex) => (
                    ...
                    >
                      ...
                      <div className="space-y-1">
                        {day.timeSlots[timeIndex]?.reservations.map(
                          (reservation) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="flex items-center justify-between mb-2">
                                  ...
                                  <span
                                    className={`px-2 py-1 rounded-full text-xs font-bold flex-shrink-0 ${getStatusColor(reservation.status)}`}
                                  >
                                ...
                                <div className="space-y-1">
                                  <div className="flex items-center gap-1 text-gray-600">
                                    ...
                                    <span className="font-medium">
                                      {reservation.number_of_people}p
                                    </span>
                                  </div>
            ...
            {weekData.map((day, dayIndex) => (
              <div key={dayIndex} className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        {day.totalReservations} reservas • {day.totalPeople}{" "}
                        pessoas
                      </p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {day.timeSlots
                    .filter(
                      (slot) => showEmptySlots || slot.reservations.length > 0,
                    )
                    .map((slot, slotIndex) => (
                      <div
                        key={slotIndex}
                        ...
                        {slot.reservations.length === 0 ? (
                          <div className="text-center py-4 text-gray-400">
                            ...
                            {!isPastDate(day.date) && (
                              <button
                                onClick={() =>
                                  onAddReservation(day.date, slot.time)
                                }
                            ...
                            {slot.reservations.map((reservation) => (
                              ...
                              >
                                <div className="flex items-center justify-between mb-2">
                                  ...
                                  <span
                                    className={`px-2 py-1 rounded text-xs font-medium border ${getStatusColor(reservation.status)}`}
                                  >
                                    {getStatusText(reservation.status)}
                                  </span>
                                </div>
