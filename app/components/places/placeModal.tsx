...
interface PlaceModalProps {
  isOpen: boolean; // Nova propriedade para controlar se o modal está aberto
...
const PlaceModal: React.FC<PlaceModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(url, {
        method: currentPlace?.id ? "PUT" : "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "Erro ao salvar o local");
      }
  ...
  return (
    ...
    >
      <h2 className="text-xl font-semibold mb-4">{currentPlace?.id ? "Editar Local" : "Adicionar Local"}</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          {currentPlace?.id ? "Atualizar Local" : "Adicionar Local"}
        </button>
      </form>
    </Modal>
  );
